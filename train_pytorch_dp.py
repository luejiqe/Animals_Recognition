"""
PyTorch DataParallel (DP) 训练脚本
适用于单机多卡训练，使用简单但性能较DDP略低
"""
import os
import numpy as np
import pandas as pd
...
# 训练函数
def train_epoch(model, train_loader, criterion, optimizer, scaler, epoch, device, use_amp):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def main():
    # 参数解析
    parser = argparse.ArgumentParser(description='Animal Classification with DataParallel')
    ...
    parser.add_argument('--batch_size', type=int, default=128, help='总批次大小(会被自动分割到各GPU)')
    ...
    print(f"=" * 80)
    print(f"DataParallel 训练配置")
    print(f"=" * 80)
    print(f"使用设备: {device}")
    print(f"GPU 数量: {len(gpu_list)}")
    print(f"GPU IDs: {gpu_list}")
    print(f"每GPU批次大小: {args.batch_size}")
    print(f"有效批次大小: {args.batch_size * len(gpu_list)}")
    ...
    # 创建数据加载器 - DP不需要DistributedSampler
    ...
    if torch.cuda.device_count() > 1:
        print(f"\n使用 DataParallel 在 {torch.cuda.device_count()} 个GPU上训练")
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            try:
                ...
                # 保存为标准 PyTorch 格式
                pytorch_model_path = 'checkpoints/pytorch_model_dp.pt'
    ...
    history_df.to_csv('training_history_dp.csv', index=False)
    ...
    plt.title('Training & Validation Loss (DP)')
    plt.legend()

    plt.subplot(2, 2, 2)
    plt.plot(history['train_acc'], label='train acc')
    plt.plot(history['val_acc'], label='val acc')
    plt.title('Training & Validation Accuracy (DP)')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig('training_metrics_dp.png', dpi=150)
    plt.close()
    ...
    metrics_df.to_csv('class_accuracy_report_dp.csv', index=False)

    print("\n" + "="*50)
    print("训练完成！")
    print(f"最佳验证损失: {best_val_loss:.4f}")
    print(f"训练历史已保存为: training_history_dp.csv")
    print(f"类别准确率报告已保存为: class_accuracy_report_dp.csv")
    print("="*50)
